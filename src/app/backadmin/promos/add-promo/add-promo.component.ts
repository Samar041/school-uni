...
export class AddPromoComponent implements OnInit {
  ...
  pubSubmit() {
    ...
    if (this.createPromoForm.valid) {
      ...
      formData.append(
        'date_debut',
        this.createPromoForm.controls['date_debut'].value + 'T00:00'
      );
      formData.append(
        'date_fin',
        this.createPromoForm.controls['date_fin'].value + 'T23:59'
      );
      formData.append(
        'value',
        this.createPromoForm.controls['pourcentage'].value
      ) + '%';
      ...
      formData.append(
        'event_type',
        this.createPromoForm.controls['event_type'].value
      );
      formData.append(
        'max_attempts',
        this.createPromoForm.controls['max_attempts'].value
      );
      formData.append(
        'type_value',
        this.createPromoForm.controls['discount_type'].value
      );

      if (this.dateMax == false) {
        this.isLoader = true;
        this.promosService.storePromos(formData).subscribe(
          (res: any) => {
            if (res.status === 201) {
