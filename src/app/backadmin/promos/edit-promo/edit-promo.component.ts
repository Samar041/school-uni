...
export class EditPromoComponent implements OnInit {
  ...
  pubSubmit() {
    ...
    if (this.editPromoForm.valid) {
      ...
      formData.append(
        'date_debut',
        this.editPromoForm.controls['date_debut'].value + 'T00:00'
      );
      formData.append(
        'date_fin',
        this.editPromoForm.controls['date_fin'].value + 'T23:59'
      );
      formData.append(
        'value',
        this.editPromoForm.controls['pourcentage'].value
      ) + '%';
      formData.append('code', this.editPromoForm.controls['code'].value);
      formData.append('titre', this.editPromoForm.controls['titre'].value);
      formData.append(
        'event_type',
        this.editPromoForm.controls['event_type'].value
      );
      formData.append(
        'max_attempts',
        this.editPromoForm.controls['max_attempts'].value
      );
      formData.append(
        'type_value',
        this.editPromoForm.controls['discount_type'].value
      );

      if (this.dateMax == false) {
        this.isLoader = true;
        this.PromosService.updatePromos(formData, this.promoId).subscribe(
          (res: any) => {
            if (res.status === 200) {
  ...
  eventTypeSelected($event: any) {
    ...
    switch ($event.value) {
      ...
      case 'SPECIFIC_CLIENTS':
        ...
        this.showList = true;
        // this.userService
        //   .userFiltred({ event_type: 'SPECIFIC_CLIENTS' })
        //   .subscribe((res: any) => {
        //     this.listFiltred = res.body.data;
        //   });
        break;
