 ...
 Providers: any = [];
  ...
  pubSubmit() {
    ...
    if (this.createAnnounceForm.valid) {
      ...
      formData.append('start_date', this.formatDate(new Date(this.createAnnounceForm.controls['date_debut'].value)));
      formData.append('end_date', this.formatDate(new Date(this.createAnnounceForm.controls['date_fin'].value)));
      formData.append('title', this.createAnnounceForm.controls['title'].value);
      formData.append('description', this.createAnnounceForm.controls['description'].value);
      formData.append('user_id', this.createAnnounceForm.controls['user_id'].value);
      formData.append('nbre_notif', this.createAnnounceForm.controls['nbre_notif'].value);
      formData.append('user_id', this.createAnnounceForm.controls['user_id'].value);

      if (this.dateMax == false) {
        this.isLoader = true;
        this.AnnounceService.storeAnnounce(formData).subscribe((res: any) => {
          if (res.status == 201) {
            this.cancel();
  ...
  updateValidation(event: any) {
    if(this.createAnnounceForm.controls['type'].value=='IMAGE'){
      this.createAnnounceForm.controls['file'].setValidators([Validators.required]);
