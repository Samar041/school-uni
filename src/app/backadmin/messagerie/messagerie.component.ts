...
export class MessagerieComponent implements OnInit {
 ...
 //// private newCommandSubscription: Subscription;
  ...
  selectChange = [
    {
      "color": "green", value: 1, name: "actif"
    },
    {
      "color": "red", value: 2, name: "inactif"
    }
  ];
  message:string="";
  messagesList!:any[];
  ...
  { id: "non-lue", name: "non-lue", value: "non-lue", label: "Non lus" , checked: false},
...
filterList:any;
 ...
 sendMessage(id: number) {
  ...
  scrollToBottom() {
    if(this.isSelected === true)this.scrollContainer.nativeElement.scrollTop = this.scrollContainer.nativeElement.scrollHeight;
  ...
  onSearch() {  
    const filteredMessages: any[] = this.clientAdminMessage.filter((message: any) =>
  ...
  handleCheckboxChange(checkedItemId: any) {
    ...
    switch (checkedItemId) {
      case "non-lue":
        filteredList = this.messagesList.filter((message: any) => message.is_read === 0);
        break;
        case "lue":
          filteredList = this.messagesList.filter((message: any) => message.is_read === 1);
          break;
      default:
        break;
    }
